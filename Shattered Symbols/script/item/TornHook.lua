...
if EID then
    EID:addCollectible(TornHookLocalID, "{{BrokenHeart}} Gives 1 Broken Heart on every Floor which does not replaces Hearts until there are no more unused slots #{{ArrowUp}} At every floor grants:#{{DamageSmall}} +0.3 Damage#{{SpeedSmall}} +0.1 Speed#{{RangeSmall}} +0.3 Range#{{TearsSmall}} +0.4 Tears#{{LuckSmall}} +0.2 Luck#{{LuckSmall}} Your Luck stat determines the chance to remove Torn Hooks each floor; on the floor Torn Hooks are removed, their effects do not activate")
end
