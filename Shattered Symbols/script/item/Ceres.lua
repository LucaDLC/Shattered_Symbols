...
if EID then
    EID:addCollectible(CeresLocalID, "{{Heart}} Heals all heart empty containers each floor #{{BrokenHeart}} Gives 1 Broken Heart which replaces Hearts in this order {{Heart}}{{BoneHeart}}{{SoulHeart}}{{BlackHeart}} #{{DeathMark}} Occasionally, an asteroid strikes a random enemy in the room; if no enemies are present, it falls at a random location. The frequency increases with the number of Broken Hearts you have #{{Warning}} Asteroids can also damage Isaac")
end
