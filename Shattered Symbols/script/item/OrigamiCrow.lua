...
if EID then
    EID:addCollectible(OrigamiCrowLocalID, "{{ArrowUp}} Moves the active item to the pocket item if you don't have one #{{ArrowUp}} If you have no active item, the first one you pick becomes the pocket item #{{BrokenHeart}} Gives 2 Broken Hearts which replaces Hearts in this order {{Heart}}{{BoneHeart}}{{SoulHeart}}{{BlackHeart}} #{{Player14}} Gives 1 Broken Hearts which replaces Hearts {{CoinHeart}} #{{Warning}} Picking up an item that grants you a pocket item will drop your current one")
end
...
local function toPocket(player)
    ...
    if player:GetActiveItem(ActiveSlot.SLOT_POCKET) ~= data.OrigamiCrowPocket and data.OrigamiCrowPocket ~= 0 and player:GetActiveItem(ActiveSlot.SLOT_POCKET) ~= 0 then
        Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, data.OrigamiCrowPocket, player.Position + Vector(0, 50), Vector(0,0), nil)
    end
...
function ShatteredSymbols:useOrigamiCrow(player)
    ...
    if not data.OrigamiCrowPocket then data.OrigamiCrowPocket = 0 end
