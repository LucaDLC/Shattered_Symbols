    ...
    for playerIndex = 0, game:GetNumPlayers() - 1 do
        ...
        if player:HasCollectible(OrigamiNightingaleLocalID) then
            ...
            if math.random(100) <= (10 + LuckIncreaseRedDoor) then
                ...
                local directions = {DoorSlot.LEFT0, DoorSlot.RIGHT0, DoorSlot.UP0, DoorSlot.DOWN0}
                local isDone = false

                -- Controlla le porte Red Room disponibili usando il flag FLAG_RED_ROOM
                for _, direction in ipairs(directions) do
                    local targetRoomDesc = level:GetRoomByIdx(currentRoomDesc.GridIndex + direction)
                    if not isDone and targetRoomDesc and targetRoomDesc.Flags & RoomDescriptor.FLAG_RED_ROOM ~= 0 then
